...
fn ids(pdk: &PdkConfig, suite: &str) -> Vec<String> {
    let mut v: Vec<String> = pdk
        .load_suite(suite)
        .unwrap()
        .iter()
        .map(|r| format!("{}::{}", r.id, r.check))
        .collect();
    v.sort();
    v
}
...
fn core_equals_main_minus_antenna_minus_density() {
    ...
    for proc in ["ihp-sg13g2", "ihp-sg13cmos5l"] {
        ...
        let expected: HashSet<String> = main
            .difference(&antenna)
            .cloned()
            .collect::<HashSet<_>>()
            .difference(&density)
        ...
        assert_eq!(
            core,
            expected,
            "[{proc}] core != main - antenna - density\n only_in_core: {:?}\n missing_from_core: {:?}",
        ...
        assert!(
            core.is_disjoint(&density),
            "[{proc}] core still has density rules"
        );
        assert!(
            core.is_disjoint(&antenna),
            "[{proc}] core still has antenna rules"
        );
        ...
        assert_eq!(
            antenna_suite, antenna,
            "[{proc}] antenna suite != antenna deck"
        );

        println!(
            "[{proc}] main={} core={} density={} antenna={}",
            main.len(),
            core.len(),
            density.len(),
            antenna.len()
    ...
    assert_eq!(
        antenna_by_proc[0], antenna_by_proc[1],
        "antenna ids differ between processes"
    );
}
